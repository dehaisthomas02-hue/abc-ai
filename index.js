...
wss.on("connection", (twilioWs) => {
  ...
  // === LOCK: une seule réponse à la fois ===
  ...
  openaiWs.on("open", () => {
    ...
    // ✅ Config stable (turn_detection server_vad)
    sendOpenAI({
      type: "session.update",
      session: {
  ...
  openaiWs.on("message", (raw) => {
    ...
    if (msg.type === "error") {
      ...
      // si une réponse est déjà active, on reste lock jusqu'à done
